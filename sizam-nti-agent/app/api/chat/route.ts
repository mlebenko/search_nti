...
Приоритет: выдай документы в порядке убывания релевантности. НЕ нужно распределять выдачу по источникам, если один источник даёт более релевантные результаты.
Сразу выдай таблицу ровно в таком виде, с переводами строк, как в примере из системного сообщения.
`.trim();
